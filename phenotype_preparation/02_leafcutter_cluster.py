###Goals(2024/1/18): Except prepare the junction ratio files; prepare corresponding PSI values
import sys
import tempfile
import os
import gzip
...
def main(options,libl):
    if options.cluster is None:
        pool_junc_reads(libl, options)  #function: libl is the junction file; 
        refine_clusters(options)
    sort_junctions(libl, options)
    merge_junctions(options)
    get_numers(options)

def pool_junc_reads(flist, options):  ##combine all the junc files together
    ...
    for chrom in by_chrom:
        read_ks = [k for k,v in list(by_chrom[chrom].items()) if v >= 3] # a junction must have at least 3 reads  read_ks is a sequence of (A,B)
        read_ks.sort()
        ...
        clu = cluster_intervals(read_ks)[0]   ##a function cluster intron with intersect
        for cl in clu:
            if len(cl) >= 1: # if cluster has more than one intron (change it to =1; intron retention)
                buf = '%s:%s '%chrom
...
def sort_junctions(libl, options):   ##function: read in refined cluster
...
def refine_linked(clusters):    #Goal: current[0][0][0] is A, current[0][0][1] is B; create newclusters 
...
def refine_cluster(clu, cutoff, readcutoff):
    ''' for each intron in the cluster compute the ratio of reads, if smaller than cutoff,
